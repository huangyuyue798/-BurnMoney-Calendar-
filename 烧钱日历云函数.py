...
昨晚上考虑了一下仅仅是通过固定的算法来计算每日固定的花销有点枯燥
我打算将算法改成 更加线性的，还是按照上面的算法来算就是580/90嘛，
580分成90份就是6.5一天然后将6.5分为90份，
例如第一天就是6.5+6.5*0.01*89 这里的89天就是前面的89天每天取出1%份出来
以此类推： 第二天就是580/90+6.5*0.01*(90-2)-6.5*(2-1)*0.01
第三天就是580/90+6.5*0.01*(90-3)-6.5*(3-1)*0.01
简化的数学表达式就是
原金额/天数范围+原金额/天数范围*0.01*(天数范围-天数)-原金额*(天数-1)*0.01
=原金额*(1+0.01*(天数范围-天数)-0.01*(天数-1))/天数范围
=原金额*(1+0.01*(天数范围-2*天数+1))/天数范围
"""
